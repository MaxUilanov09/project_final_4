...
function fillModal(data) {
    ...
    const image = event.images.toSorted((a, b)=>Math.abs(700 - a.height) - Math.abs(700 - b.height))[0].url || event.images?.find((img)=>img.ratio === "4_3")?.url || event.images?.[0]?.url || "";
...
const createPagItem = (pageNum, accent = false)=>{
    if (pageNum !== '>' && pageNum !== '<') return `<div data-pageid="${pageNum}" class="pagination__item${accent ? ' pagination_accent' : ''}"><p style="transform: translateY(-2px)">${pageNum}</p></div>`;
    else if (pageNum === '>') return `<div data-pageid="${pageNum}" class="pagination__item${accent ? ' pagination_accent' : ''}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg></div>`;
    else if (pageNum === '<') return `<div data-pageid="${pageNum}" class="pagination__item${accent ? ' pagination_accent' : ''}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg></div>`;
};
